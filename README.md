...
#### 7. Если при получении баннера передан флаг use_last_revision, возвращается актуальная информация. В ином случае происходит передача информации, которая была актуальна 5 минут назад.
...
Использовал Apache JMeter. Настроил его, создал 100 пользователей в CSV файле, загрузил, сделал запросы на получение токенов. В БД создались пользователи и следующие запросы происходили с получением токена(~235 мс), а не создания(~11 мс).
Тестирование по факту двух путей, получение токена и выполнение запроса с токеном.

Настройки:

количество пользователей (потоков) - 100;

количество итераций - 10;

время нарастания - 1 минута;

Результаты из листенера Summary Report:
![image](https://github.com/electr0n4ik/banner_service/assets/116460003/62def33e-285a-4262-a516-38bb114117c4)

Следующий тест сделал 100 итераций и период нарастания 3 минуты. Добавил отрисовку графика с интервалом в 5 мс.

![image](https://github.com/electr0n4ik/banner_service/assets/116460003/a03e0a36-4cc1-4587-95f1-572c4a982d6e)

![image](https://github.com/electr0n4ik/banner_service/assets/116460003/47fd280c-53eb-4bbd-acf7-e2e2d71316b0)

Samples: Общее количество выполненных операций.

Average: Среднее время выполнения операции в мс.

Min: Минимальное время выполнения операции в мс.

Max: Максимальное время выполнения операции в мс.

Std. Dev.: Стандартное отклонение времени выполнения операции.

Error %: Процент ошибок в выполнении операции.

Throughput: Количество операций, выполненных за единицу времени (обычно в секундах).

Received KB/sec: Скорость получения данных.

Sent KB/sec: Скорость отправки данных.

Avg. Bytes: Средний размер операции.
